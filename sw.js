...
self.addEventListener('fetch', function (event) {
  ...
  event.respondWith(caches.open(CACHE_NAME).then((cache) => {
    return cache.match(event.request).then((cachedVersion) => {
      return cachedVersion || fetch(event.request).then((fetchedVersion) => {
        cache.put(event.request, fetchedVersion.clone());
